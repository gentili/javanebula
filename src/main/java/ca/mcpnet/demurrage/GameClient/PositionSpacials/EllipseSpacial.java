...
public class EllipseSpacial extends Node {
	...
	private void ellipseInit() {
		...
		_ellipseGeom.setMaterial(new Material(_assetManager,
				"Common/MatDefs/Misc/SolidColor.j3md"));
	...
	private void pathInit(float trueAnom) {
		...
		for (int curpoint = 0; curpoint < _pathPointCount; curpoint++) {
			float E = eccentricAnom
					- (float) (curpoint / (float) _pathPointCount * Math.PI / 2);
			...
			colors[colori++] = 1.1f - (float) curpoint
					/ (float) (_pathPointCount); // GREEN
		...
		_pathGeom.setMaterial(new Material(_assetManager,
	...
	private Vector3f getPointAtTrueAnomaly(float theta) {
		float r = (float) (_semiMajorAxis * (1 - _e * _e) / (1 + _e
				* FastMath.cos(theta)));
		return new Vector3f((float) (r * FastMath.cos(theta)),
				(float) (r * FastMath.sin(theta)), (float) 0.0);
	}
